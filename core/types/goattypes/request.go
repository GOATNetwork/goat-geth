...
const (
	...
	Cancel1RequestType
	DepositTaxRequestType
	ConfirmationNumberRequestType
	MinDepositRequestType
	_
	_
	_
	AddVoterRequestType
...
var (
	...
	ConfirmationNumberEventTopic = common.HexToHash("0x30b92002139b64ec601b714d1ecccba1212034e735773b3de088e8876f4dfb65")
	UpdateMinDepositEventTopic   = common.HexToHash("0x7aa20a242ea0b0f7b0141c56aaad636eb2b2077c9c27d09a8282f6931f486a21")
	UpdateDepositTaxEventTopic   = common.HexToHash("0x1007ff7aec53e9626ce51f25d4e093f290f60da8019c8cf489f0ae2f21ebf76a")
)
...
func DecodeRequests(reqs [][]byte) (bridge BridgeRequests, relayer RelayerRequests, locking LockingRequests, err error) {
	...
	for i := 0; i < len(reqs); i++ {
		...
		case DepositTaxRequestType:
			for reader.Len() != 0 {
				inner := new(DepositTaxRequest)
				if err = inner.DecodeReader(reader); err != nil {
					return
				}
				bridge.DepositTax = append(bridge.DepositTax, inner)
			}
		case ConfirmationNumberRequestType:
			for reader.Len() != 0 {
				inner := new(ConfirmationNumberRequest)
				if err = inner.DecodeReader(reader); err != nil {
					return
				}
				bridge.Confirmation = append(bridge.Confirmation, inner)
			}
		case MinDepositRequestType:
			for reader.Len() != 0 {
				inner := new(MinDepositRequest)
				if err = inner.DecodeReader(reader); err != nil {
					return
				}
				bridge.MinDeposit = append(bridge.MinDeposit, inner)
			}
