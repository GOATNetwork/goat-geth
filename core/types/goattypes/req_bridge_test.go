...
func TestUnpackIntoDepositTaxRequest(t *testing.T) {
	type args struct {
		data []byte
	}
	tests := []struct {
	...
	}{
		{
			...
			want: &DepositTaxRequest{Rate: 2, Max: 10000},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := UnpackIntoDepositTaxRequest(tt.args.data)
			if (err != nil) != tt.wantErr {
				t.Errorf("UnpackIntoDepositTaxRequest() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("UnpackIntoDepositTaxRequest() = %v, want %v", got, tt.want)
				return
			}

			if !reflect.DeepEqual(got, got.Copy()) {
				t.Errorf("UnpackIntoDepositTaxRequest() copy notDeepEqual= %v, want %v", got, tt.want)
				return
			}

			if got.RequestType() != DepositTaxRequestType {
				t.Errorf("UnpackIntoDepositTaxRequest() inconsistent requestType: %d", got.RequestType())
				return
			}

			newReq := new(DepositTaxRequest)
			if err := newReq.Decode(got.Encode()); err != nil {
				t.Errorf("UnpackIntoDepositTaxRequest(): codec: %s", err)
				return
			}

			if !reflect.DeepEqual(got, newReq) {
				t.Errorf("UnpackIntoDepositTaxRequest(): not deepEqual: = %v, want %v", got, newReq)
				return
			}

			newReq2 := new(DepositTaxRequest)
			if err := newReq2.DecodeReader(bytes.NewReader(got.Encode())); err != nil {
				t.Errorf("UnpackIntoDepositTaxRequest(): DecodeReader: %s", err)
				return
			}

			if !reflect.DeepEqual(got, newReq2) {
				t.Errorf("UnpackIntoDepositTaxRequest(): DecodeReader: not deepEqual: = %v, want %v", got, newReq2)
				return
			}

			eventTopic := common.Hash(crypto.Keccak256([]byte("DepositTaxUpdated(uint16,uint64)")))
			if eventTopic != UpdateDepositTaxEventTopic {
				t.Errorf("invalid DepositTaxRequestType event topic")
			}
		})
	}
}

func TestUnpackIntoConfirmationNumberRequest(t *testing.T) {
	type args struct {
		data []byte
	}
	tests := []struct {
		name    string
		args    args
		want    *ConfirmationNumberRequest
		wantErr bool
	}{
		{
			name: "1",
			...
			want: &ConfirmationNumberRequest{Number: 6},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := UnpackIntoConfirmationNumberRequest(tt.args.data)
			if (err != nil) != tt.wantErr {
				t.Errorf("UnpackIntoConfirmationNumberRequest() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("UnpackIntoConfirmationNumberRequest() = %v, want %v", got, tt.want)
				return
			}

			if !reflect.DeepEqual(got, got.Copy()) {
				t.Errorf("UnpackIntoConfirmationNumberRequest() copy notDeepEqual= %v, want %v", got, tt.want)
				return
			}

			if got.RequestType() != ConfirmationNumberRequestType {
				t.Errorf("UnpackIntoConfirmationNumberRequest() inconsistent requestType: %d", got.RequestType())
				return
			}

			newReq := new(ConfirmationNumberRequest)
			if err := newReq.Decode(got.Encode()); err != nil {
				t.Errorf("UnpackIntoConfirmationNumberRequest(): codec: %s", err)
				return
			}

			if !reflect.DeepEqual(got, newReq) {
				t.Errorf("UnpackIntoConfirmationNumberRequest(): not deepEqual: = %v, want %v", got, newReq)
				return
			}

			newReq2 := new(ConfirmationNumberRequest)
			if err := newReq2.DecodeReader(bytes.NewReader(got.Encode())); err != nil {
				t.Errorf("UnpackIntoConfirmationNumberRequest(): DecodeReader: %s", err)
				return
			}

			if !reflect.DeepEqual(got, newReq2) {
				t.Errorf("UnpackIntoConfirmationNumberRequest(): DecodeReader: not deepEqual: = %v, want %v", got, newReq2)
				return
			}

			eventTopic := common.Hash(crypto.Keccak256([]byte("ConfirmationNumberUpdated(uint16)")))
			if eventTopic != ConfirmationNumberEventTopic {
				t.Errorf("invalid ConfirmationNumberRequestType event topic")
			}
		})
	}
}

func TestUnpackIntoMinDepositRequest(t *testing.T) {
	type args struct {
		data []byte
	}
	tests := []struct {
	...
	}{
		{
			...
			want: &MinDepositRequest{Satoshi: 1e4},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := UnpackIntoMinDepositRequest(tt.args.data)
			if (err != nil) != tt.wantErr {
				t.Errorf("UnpackIntoMinDepositRequest() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("UnpackIntoMinDepositRequest() = %v, want %v", got, tt.want)
				return
			}

			if !reflect.DeepEqual(got, got.Copy()) {
				t.Errorf("UnpackIntoMinDepositRequest() copy notDeepEqual= %v, want %v", got, tt.want)
				return
			}

			if got.RequestType() != MinDepositRequestType {
				t.Errorf("UnpackIntoMinDepositRequest() inconsistent requestType: %d", got.RequestType())
				return
			}

			newReq := new(MinDepositRequest)
			if err := newReq.Decode(got.Encode()); err != nil {
				t.Errorf("UnpackIntoMinDepositRequest(): codec: %s", err)
				return
			}

			if !reflect.DeepEqual(got, newReq) {
				t.Errorf("UnpackIntoMinDepositRequest(): not deepEqual: = %v, want %v", got, newReq)
				return
			}

			newReq2 := new(MinDepositRequest)
			if err := newReq2.DecodeReader(bytes.NewReader(got.Encode())); err != nil {
				t.Errorf("UnpackIntoMinDepositRequest(): DecodeReader: %s", err)
				return
			}

			if !reflect.DeepEqual(got, newReq2) {
				t.Errorf("UnpackIntoMinDepositRequest(): DecodeReader: not deepEqual: = %v, want %v", got, newReq2)
				return
			}

			eventTopic := common.Hash(crypto.Keccak256([]byte("MinDepositUpdated(uint64)")))
			if eventTopic != UpdateMinDepositEventTopic {
				t.Errorf("invalid MinDepositRequestType event topic")
			}
		})
	}
}
