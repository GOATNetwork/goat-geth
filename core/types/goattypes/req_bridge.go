...
func (req *WithdrawalRequest) DecodeReader(reader io.Reader) error {
	...
	if n, err := reader.Read(input); err != nil {
		...
		return errors.New("invalid WithdrawalRequest bytes length")
	}
	res, err := DecodeUint64(input[:24], 3)
	if err != nil {
		return err
	...
	if n, err := reader.Read(address); err != nil {
		return err
	} else if n != int(input[24]) {
		return errors.New("invalid WithdrawalRequest.Address bytes length")
	}
...
func (req *Cancel1Request) DecodeReader(reader io.Reader) error {
	...
	if n, err := reader.Read(input); err != nil {
		...
		return errors.New("invalid Cancel1Request bytes length")
	}
	return req.Decode(input)
}
