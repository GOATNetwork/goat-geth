...
func (tx *DepositTx) Decode(input []byte) error {
	...
	input = input[32:]
	tx.Tax = new(big.Int).SetBytes(input[:32])
	return nil
}
...
func (tx *DepositTx) Deposit() *Mint {
	return &Mint{tx.Target, new(big.Int).Set(tx.Amount), new(big.Int).Set(tx.Tax)}
}

func (tx *DepositTx) Withdraw() *Mint {
	return nil
}
...
func (tx *Cancel2Tx) Withdraw() *Mint {
	return nil
}
...
type NewBtcBlockTx struct {
	Hash common.Hash
}

func (tx *NewBtcBlockTx) Size() int {
	return 36
}

func (tx *NewBtcBlockTx) isGoatTx() {}

func (tx *NewBtcBlockTx) Copy() Tx {
	return &NewBtcBlockTx{Hash: tx.Hash}
}

func (tx *NewBtcBlockTx) MethodId() [4]byte {
	// newBlockHash(bytes32 hash)
	return [4]byte{0x94, 0xf4, 0x90, 0xbd}
}

func (tx *NewBtcBlockTx) Encode() []byte {
	b := make([]byte, 0, tx.Size())
...
func (tx *NewBtcBlockTx) Decode(input []byte) error {
	if len(input) != tx.Size() {
		return errors.New("Invalid input data for newBitcoinHash tx")
	}
	if [4]byte(input[:4]) != tx.MethodId() {
		return errors.New("not a newBlockHash tx")
	}
	tx.Hash = common.BytesToHash(input[4:])
	return nil
}

func (tx *NewBtcBlockTx) Sender() common.Address {
	return RelayerExecutor
}

func (tx *NewBtcBlockTx) Contract() common.Address {
	return BitcoinContract
}

func (tx *NewBtcBlockTx) Deposit() *Mint {
	return nil
}

func (tx *NewBtcBlockTx) Withdraw() *Mint {
	return nil
}
