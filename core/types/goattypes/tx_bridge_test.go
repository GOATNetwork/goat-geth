...
func TestDepositTx(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			methodId := [4]byte(crypto.Keccak256([]byte("deposit(bytes32,uint32,address,uint256)"))[:4])
...
func TestNewBtcBlockTx(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tx.Claim() != nil {
				t.Errorf("NewBtcBlockTx.Claim() != nil")
			}
