...
func (tx *CompleteUnlockTx) Withdraw() *Mint {
	if tx.Token == NativeToken {
		return &Mint{tx.Recipient, new(big.Int).Set(tx.Amount), new(big.Int)}
	}
	return nil
}
...
func (tx *DistributeRewardTx) Withdraw() *Mint {
	return &Mint{tx.Recipient, new(big.Int).Set(tx.GasReward), new(big.Int)}
}
