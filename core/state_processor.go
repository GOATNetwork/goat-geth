...
func (p *StateProcessor) Process(ctx context.Context, block *types.Block, statedb *state.StateDB, cfg vm.Config) (*ProcessResult, error) {
	...
	for i, tx := range block.Transactions() {
		...
		if receipt.GasUsed > 0 { // non-goatTx case
			...
			tipFee, err := tx.EffectiveGasTip(context.BaseFee)
			if err != nil {
				return nil, fmt.Errorf("could not calculate tip fee for tx %d [%v]: %w", i, tx.Hash().Hex(), err)
			}
			tipFee.Mul(tipFee, gasUsed)
	...
	requests, err := postExecution(ctx, config, block, allLogs, goatGasFees, evm)
	if err != nil {
		return nil, err
	}
...
func postExecution(ctx context.Context, config *params.ChainConfig, block *types.Block, allLogs []*types.Log, goatGasFees *big.Int, evm *vm.EVM) (requests [][]byte, err error) {
	...
	} else if config.IsPrague(block.Number(), block.Time()) {
		requests = [][]byte{}
		// EIP-6110
